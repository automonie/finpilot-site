// Automonie Money Personality Quiz. Maps 10 answers to one of the 8 spending
// archetypes (mirrors finpilot-mobile/src/lib/archetype.ts). Pure client-side,
// no dependency. Animated transitions + a result reveal. Captures a waitlist
// email at the end.
(function () {
  ...
  var QUESTIONS = [
    ...
    { q: 'A friend asks to borrow money, three days to payday. You...', a: [
      { t: 'Send it, figure mine out later.', w: { detty: 1, ghost: 1 } },
      { t: "Politely say I'm also managing.", w: { sapa: 2 } },
      { t: 'Send it, I planned a buffer for this.', w: { ajo: 1, steady: 1 } },
      { t: "I don't even know if I can spare it.", w: { ghost: 1, subs: 1 } },
    ] },
    { q: 'Your ideal free weekend?', a: [
      { t: 'Brunch, cinema, a nice outing.', w: { softlife: 2 } },
      { t: 'Turn-up, drinks, the full detty.', w: { detty: 2 } },
      { t: 'Home and rested. Rest is free.', w: { steady: 1, ajo: 1, sapa: 1 } },
      { t: "Anything, as long as I'm not counting cash.", w: { ghost: 1, detonator: 1 } },
    ] },
    { q: 'How often do you actually check your balance?', a: [
      { t: 'Constantly, I know it to the naira.', w: { sapa: 1, ajo: 1 } },
      { t: 'Only when the alert scares me.', w: { detonator: 1, ghost: 1 } },
      { t: 'Weekly-ish, I like to stay aware.', w: { steady: 2 } },
      { t: 'I avoid it. Ignorance is peace.', w: { ghost: 2 } },
    ] },
    { q: 'Pick your money mantra:', a: [
      { t: '"Save first, flex later."', w: { ajo: 2 } },
      { t: '"You only live once."', w: { detty: 2 } },
      { t: '"Soft life, but calculated."', w: { softlife: 2 } },
      { t: '"Whatever remains is savings."', w: { ghost: 1, detonator: 1 } },
    ] },
  ];
  ...
  function renderQuestion() {
    ...
    Q.a.forEach(function (opt, i) {
      opts += '<button class="q-opt" data-i="' + i + '" style="animation-delay:' + (80 + i * 70) + 'ms">' + opt.t + '</button>';
    });
    stage.innerHTML =
      '<div class="q-anim">' +
        ...
        (idx > 0 ? '<button class="q-back" id="qBack">Back</button>' : '') +
      '</div>';
    stage.scrollIntoView({ behavior: 'smooth', block: 'start' });

    var os = stage.querySelectorAll('.q-opt');
    for (var j = 0; j < os.length; j++) {
      os[j].addEventListener('click', function () { pick(this, +this.getAttribute('data-i')); });
    }
    var back = el('qBack');
  ...
  function pick(btn, i) {
    var all = stage.querySelectorAll('.q-opt');
    for (var j = 0; j < all.length; j++) all[j].disabled = true;
    btn.classList.add('sel');
    setTimeout(function () { choose(i); }, 280);
  }
  ...
  function choose(i) {
    ...
    idx++;
    if (idx >= QUESTIONS.length) interstitial(); else renderQuestion();
  }
  ...
  function interstitial() {
    stage.innerHTML =
      '<div class="q-anim q-loading">' +
        '<div class="q-dots"><span></span><span></span><span></span></div>' +
        '<p class="q-loading-text">Reading your spending DNA...</p>' +
      '</div>';
    stage.scrollIntoView({ behavior: 'smooth', block: 'start' });
  ...
  function renderResult() {
    ...
    stage.innerHTML =
      '<div class="q-anim q-result" style="--acc:' + A.color + '">' +
        ...
        '<div class="q-eyebrow r1">Your money personality</div>' +
        '<h2 class="q-name r2">' + A.name + '</h2>' +
        '<p class="q-tag r3">' + A.tagline + '</p>' +
        '<p class="q-blurb r4">' + A.blurb + '</p>' +
        '<div class="q-actions r5">' +
          '<button class="btn btn-primary" id="qShare">Share my result</button>' +
          '<button class="q-retake" id="qRetake">Retake quiz</button>' +
        '</div>' +
        '<div class="q-capture r6">' +
  ...
  function share(A) {
    var text = 'I am ' + A.name + '. "' + A.tagline + '" What is your money personality? Take the Automonie quiz:';
